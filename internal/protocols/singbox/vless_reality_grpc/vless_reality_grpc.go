...
func Inbound(ctx shared.Context, port int) (map[string]any, error) {
	...
	inbound["tag"] = shared.Tag("vless-reality-grpc", port)
...
func ConnectionLink(ctx shared.Context, port int, name string) string {
	return fmt.Sprintf("vless://%s@%s:%d?security=reality&sni=%s&pbk=%s&sid=%s&type=grpc&serviceName=%s#%s", ctx.UUID, ctx.NodeIP, port, ctx.RealityServer, ctx.RealityPublicKey, ctx.RealityShortID, ctx.GRPCServiceName, name)
}
